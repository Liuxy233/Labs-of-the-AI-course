...
class MazeRLAgent(object):
    ...
    def policy_evaluation(self, env, values, policy):
        while True:
            ...
            for state in env.legel_states:
                action = policy[state]
                for prob, next_state, reward, done in env.transition(state, action):
                    if not done:
                        new_values[state] += prob * (reward + self.gamma * values[next_state])
                    else:
                        new_values[state] += prob* reward
                difference += abs(new_values[state] - values[state])
    ...
    def policy_iteration(self, env):
        ...
        while True:
            ...
            for state in env.legel_states:
                pi_s = policy[state]
                prob, next_state,reward, done = env.transition(state, pi_s)[0]
                if done:
                    b = reward
                else:
                    b = reward+self.gamma*values[next_state]
                action_preferred = 0
                a = -1000
                for action in env.legal_actions(state):
                    ...
                    for prob, next_state,reward, done in env.transition(state, action):
                        if not done:
                            temp += prob*(reward+self.gamma*values[next_state])
                        else:
                            temp += prob * reward
                    if temp >= a:
                        action_preferred = action
                    a = max(a, temp)
                if a > b:
                    policy[state] = action_preferred
                    unchanged = False
    ...
    def value_iteration(self, env):
        ...
        while True:
            ...
            for state in env.legel_states:
                ...
                action_preferred = actions[0]
                temp = -10000
                for action in actions:
                    for prob, next_state, reward, _ in env.transition(state, action):
                        s = prob*(reward + self.gamma * values[next_state])
                    if s >= temp:
                        action_preferred = action
                        temp = s
                new_values[state] = temp
                policy[state] = action_preferred 
                difference += abs(new_values[state] - Us)
            self.iteration_number += 1
            if difference < 1e-4: break
            else: 
                values = new_values
