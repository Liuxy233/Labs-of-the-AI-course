...
def get_value(node, alpha, beta):
    ...
    if node.is_leaf:
        ...
        if node.rule == 'max': # 该节点要取子节点中的最大值MAX
...
def max_value(node, alpha, beta):
    ...
    for a in node.successor:
        v = max(v,min_value(get_value(a,alpha,beta)),alpha,beta)
        if v>=beta:
            return v
        alpha = max(alpha,v)
    return v
...
def min_value(node, alpha, beta):
    ...
    v = float('inf')
    for a in node.successor:
        v = min(v,max_value(get_value(a,alpha,beta),alpha,beta))
        if v<=alpha:
            return v
        beta = min(beta,v)
    return v
...
def get_unvisited_nodes(node):
    ...
    Returns:
        float list of values of the unvisited nodes. # 没有访问过的叶子节点（剪枝）的值
    """
    unvisited = []
    # 若node有后继节点，就把后面的所有节点加进去
    if node.successor:
