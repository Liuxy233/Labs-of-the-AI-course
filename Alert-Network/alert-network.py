...
class BayesNode:
    ...
    def p(self, value, event):
        ...
        value of X and all evidence(a dist)"""
...
def enumeration_ask(X, e, bn):
    ...
    for item in [True, False]:
        e2 = extend(e,X,item)
        Q[item] = enumerate_all(bn.variables, e2, bn)
    return Q.normalize()
    # END_YOUR_CODE
...
def enumerate_all(variables, e, bn):
    ...
    for item in values_list:
        total += Ynode.p(item, e) * enumerate_all(rest, extend(e, Y, item), bn)
    return total
    # END_YOUR_CODE
...
def elimination_ask(X, e, bn, order=reversed):
    ...
    factors = []
    evidence_name = e.keys()
    for item in order(bn.variables):
        factors.append(make_factor(item, e, bn))
        if item not in evidence_name and item not in X: ## X是list
            factors = sum_out(item,factors)
    tmp = pointwise_product(factors)
    prob_evidence = tmp[(True,)] + tmp[(False,)]
    return tmp.normalize(), prob_evidence
    # END_YOUR_CODE
...
def sum_out(var, factors):
    ...
    for f in factors:
        if var in f.variables:
            var_factors.append(f) #var_factors存储要做逐点加法的factor
        else:
            result.append(f) #result存储不含有var的factor
